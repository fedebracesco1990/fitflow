...
export class StatsService {
  ...
  async getExerciseProgress(
    ...
    const rawData = await this.exerciseLogRepository
      ...
      .andWhere('el.exerciseId = :exerciseId', { exerciseId })
      .andWhere('wl.status = :status', { status: WorkoutStatus.COMPLETED })
      .andWhere('wl.startedAt BETWEEN :start AND :end', { start, end })
      .andWhere('el.completed = :completed', { completed: true })
      .select('DATE(wl.startedAt)', 'date')
  ...
  private async getMonthStats(
    ...
    const result = await this.workoutLogRepository
      ...
      .innerJoin('wl.userProgramRoutine', 'upr')
      .innerJoin('upr.userProgram', 'up')
      .leftJoin('wl.exerciseLogs', 'el', 'el.completed = :completed', { completed: true })
      .where('up.userId = :userId', { userId })
      .andWhere('wl.status = :status', { status: WorkoutStatus.COMPLETED })
      .andWhere('wl.startedAt BETWEEN :start AND :end', { start, end })
      ...
      .addSelect('COUNT(DISTINCT el.exerciseId)', 'uniqueExercises')
